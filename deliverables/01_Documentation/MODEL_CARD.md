...
- **Mitigation implemented for Sex and Age:** group-specific FPR-equalizing thresholds raised disparate impact for Sex (0.83 → 0.98) and Age (0.56 → 0.86, halving the young-applicant wrongful-denial rate) at a small recall cost (~0.70 → 0.62–0.66).
- **Foreign worker — a documented trade-off, not a fix:** the same technique lifts disparate impact (0.73 → 0.88) but collapses overall recall (0.70 → 0.50), because foreign workers are ~96% of applicants. This requires a policy/reweighing approach, not an automatic threshold — the top remaining fairness task.
