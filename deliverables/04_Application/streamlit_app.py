...
from recourse import find_recourse  # noqa: E402
...
# --- Path to approval — algorithmic recourse -------------------------------
if p_bad >= 0.5:
    rec = find_recourse(pipe, row, threshold=0.5)
    st.subheader("Path to approval (algorithmic recourse)")
    if rec["status"] == "recourse":
        st.caption("The smallest realistic change to the requested loan that would "
                   "flip this to an approval — a denial should come with a path, not "
                   "just a reason.")
        for o in rec["options"]:
            risk = f"risk falls to {o['p_bad']:.0%}"
            if o["feature"] == "Credit amount":
                st.success(f"**Reduce the loan amount** from {o['from']:,.0f} to "
                           f"{o['to']:,.0f} DM (−{o['reduction_pct']}%) → approved ({risk})")
            elif o["feature"] == "Duration":
                st.success(f"**Shorten the term** from {o['from']:.0f} to {o['to']:.0f} "
                           f"months (−{o['reduction_pct']}%) → approved ({risk})")
            else:
                st.success(f"**Reduce both the amount and the term by {o['reduction_pct']}%** "
                           f"→ approved ({risk})")
    else:
        st.info("No simple reduction to the loan size or term flips this to an approval. "
                "This application needs a broader manual review — not just a smaller loan.")
